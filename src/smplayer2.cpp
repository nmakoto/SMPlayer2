...
SMPlayer2::SMPlayer2(const QString &config_path, QObject *parent)
    : QObject(parent)
{
    main_window = 0;
...
BaseGui *SMPlayer2::gui()
{
    if (main_window == 0) {
        // Changes to app path, so smplayer2 can find a relative mplayer path
        QDir::setCurrent(Paths::appPath());
        qDebug("SMPlayer2::gui: changed working directory to app path");
        ...
        else if (gui_to_use.toLower() == "mpcgui")
        ...
        if (resize_gui) {
            qDebug("SMPlayer2::gui: resizing main window to %dx%d", gui_size.width(), gui_size.height());
...
SMPlayer2::ExitCode SMPlayer2::processArgs(QStringList args)
{
    qDebug("SMPlayer2::processArgs: arguments: %d", args.count());

    for (int n = 0; n < args.count(); n++) {
        qDebug("SMPlayer2::processArgs: %d = %s", n, args[n].toUtf8().data());
    }
    ...
    for (int n = 1; n < args.count(); n++) {
        ...
        } else if (argument == "-disable-server") {
            use_control_server = false;
        } else if (argument == "-add-to-playlist") {
            add_to_playlist = true;
        } else if (argument == "-mini" || argument == "-minigui") {
            gui_to_use = "MiniGui";
        } else if (argument == "-mpcgui") {
            gui_to_use = "MpcGui";
    ...
    qDebug("SMPlayer2::processArgs: files_to_play: count: %d", files_to_play.count());

    for (int n = 0; n < files_to_play.count(); n++) {
        qDebug("SMPlayer2::processArgs: files_to_play[%d]: '%s'", n, files_to_play[n].toUtf8().data());
    }
    ...
    if (pref->use_single_instance) {
        ...
        //c->setTimeOut(1000);
        qDebug("SMPlayer2::processArgs: trying to connect to port %d", port);

        if (c->openConnection()) {
            qDebug("SMPlayer2::processArgs: found another instance");

            if (!action.isEmpty()) {
                if (c->sendAction(action)) {
                    qDebug("SMPlayer2::processArgs: action passed successfully to the running instance");
                } else {
                ...
                if (!subtitle_file.isEmpty()) {
                    if (c->sendSubtitleFile(subtitle_file)) {
                        qDebug("SMPlayer2::processArgs: subtitle file sent successfully to the running instance");
                    } else {
                        qDebug("SMPlayer2::processArgs: subtitle file couldn't be sent to another instance");
                    }
                }

                if (!files_to_play.isEmpty()) {
                    if (c->sendFiles(files_to_play, add_to_playlist)) {
                        qDebug("SMPlayer2::processArgs: files sent successfully to the running instance");
                        qDebug("SMPlayer2::processArgs: exiting.");
                    } else {
                        qDebug("SMPlayer2::processArgs: files couldn't be sent to another instance");
                    }
                }
            }
    ...
    return SMPlayer2::NoExit;
}

void SMPlayer2::start()
{
...
void SMPlayer2::createConfigDirectory()
{
    ...
    if (!QFile::exists(Paths::configPath())) {
        ...
        if (!d.mkdir(Paths::configPath())) {
            qWarning("SMPlayer2::createConfigDirectory: can't create %s", Paths::configPath().toUtf8().data());
        }

        QString s = Paths::configPath() + "/screenshots";

        if (!d.mkdir(s)) {
            qWarning("SMPlayer2::createHomeDirectory: can't create %s", s.toUtf8().data());
        }
    }
}
#endif

void SMPlayer2::showInfo()
{
#ifdef Q_OS_WIN
    QString win_ver;
